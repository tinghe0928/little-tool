...
if sys.maxunicode > 0x10000:
    _char_tail = version_adapted.u('%s-%s') % (
        version_adapted.unichr(0x10000),
        version_adapted.unichr(min(sys.maxunicode, 0x10FFFF))
    )

_nontext_sub = re.compile(
    version_adapted.u(r'[^\x09\x0A\x0D\x20-\uD7FF\uE000-\uFFFD%s]') % _char_tail,
    re.U
).sub


def replace_nontext(text, replacement=version_adapted.u('\uFFFD')):
    ...
    def _set_result_counters(self):
        ...
        for counter_name in TestXMLContext._allowed_counters:
            ...
            if valid_counter_for_element:
                value = version_adapted.text_type(
...
class TestXMLBuilder(object):
    ...
    def append(self, tag, content, **kwargs):
        ...
        for key, value in kwargs.items():
            filtered_value = replace_nontext(version_adapted.text_type(value))
