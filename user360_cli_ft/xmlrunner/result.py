...
import re
from os import path
from version_adapted.moves import StringIO
...
_illegal_ranges = [
    "%s-%s" % (version_adapted.unichr(low), version_adapted.unichr(high))
...
def xml_safe_unicode(base, encoding='utf-8'):
    ...
    encoding - if base is a byte string it is first decoded to unicode
        using this encoding.
    """
    if isinstance(base, version_adapted.binary_type):
...
def to_unicode(data):
    ...
    if version_adapted.PY3:
        return version_adapted.text_type(data)
    try:
        # Try utf8
        return version_adapted.text_type(data)
...
class _XMLTestResult(_TextTestResult):
    ...
    def generate_reports(self, test_runner):
        ...
        outputHandledAsString = \
            isinstance(test_runner.output, version_adapted.string_types)
